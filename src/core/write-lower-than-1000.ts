...
import listFrom20To90By10 from './lists/list-from-20-to-90-by-10'
import listFrom100To900By100 from './lists/list-from-100-to-900-by-100'
...
const writeLowerThan100 = (input: number): string => {
    ...
    const name = listFrom20To90By10[(input - input % 10) / 10 - 2]
    if (input % 10 === 0) {
        return name
    }
...
const writeLowerThan1000 = (input: number): string => {
    ...
    const name = listFrom100To900By100[(input - input % 100) / 100 - 1]
    if (input % 100 === 0) {
        if (input === 100) {
            return 'cem'
        }
